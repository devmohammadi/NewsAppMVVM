...
import com.fmohammadi.newsappmvvm.util.Constants
import com.fmohammadi.newsappmvvm.util.Constants.Companion.SEARCH_NEWS_TIME_DELAY
...
class SearchNewsFragment : Fragment(R.layout.fragment_search_news) {
            ...
            job = MainScope().launch {
                ...
                editable?.let {
                    if (editable.toString().isNotEmpty()) {
                        viewModels.searchNews(editable.toString())
                    }
                }
            }
        }

        viewModels.searchNews.observe(viewLifecycleOwner, Observer { response ->
            when (response) {
                is Resource.Success -> {
                    hideProgressBar()
                    response.data?.let { newsResponse ->
                        ...
                        isLastPage = viewModels.searchNewsPage == totalPages
                        if(isLoading){
    ...
    private val scrollListener = object : RecyclerView.OnScrollListener() {
        override fun onScrolled(recyclerView: RecyclerView, dx: Int, dy: Int) {
            ...
            if (shouldPaginate) {
                viewModels.searchNews(etSearch.text.toString())
                isScrolling = false
            }
        }
